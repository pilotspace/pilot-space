...
import { useEffect, useRef, useState, useMemo, type ReactPortal } from 'react';
...
export function useMonacoViewZones(
  ...
  useEffect(() => {
    ...
    // Update positions for existing zones
    manager.updatePositions(blocks);
    ...
    const newPortals = blocks
      .map((block, i) => {
        ...
        return createPortal(
          PMBlockViewZone({ type: block.type, data: block.data, raw: block.raw }),
          domNode,
          blockId
        );
      })
